      ...
      <Link href={`/posts/${id}`}>
        <article className='flex-col gap-2'>
          <div className='relative w-full h-36' role="img">
            <Image loading='lazy' className='rounded rounded-b-none' fill src={image} alt={title} objectFit='cover' />
          </div>
          <div className='p-4'>
            ...
            <div className='flex justify-between items-end'>
              ...
              <time className='text-xs text-gray-400' dateTime={createdAt}>{createdAt}</time>
            </div>
          </div>
        </article>
      </Link>
