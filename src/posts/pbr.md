...
사실 반사광에 대해 정확하게 구현하고 사전 필터링을 하기 위해서는 뷰 벡터 $V$ 에 대한 정보가 있어야 합니다. 하지만, 이 시점에서는 뷰 벡터에 대한 정보가 없기 때문에, 이를 단순화하기 위해 한 가지를 가정합니다. 바로 법선/반사/뷰 벡터가 모두 동일하다( $V=R=N$ )는 가정을 두어서 이를 근사하는 것입니다. 이를 통해 사전 필터링 단계에서 뷰 벡터에 대한 정보 자체가 필요없게 만듭니다.
