...
- $L$ : Radiance (복사휘도/방사휘도), 특정 지점에서 들어오거나 나가는 빛의 세기에 대한 측정입니다.
- $\Omega$ : Hemisphere (반구), 노멀 벡터 $n$ 을 기준으로 한 모든 입사 방향 $\omega_i$ 의 집합
- $\omega_i$, $\omega_o$ : 각각 입사/출사 방향 벡터

![Flux, Irradiance, Radiance](image-8.png)

PBR은 이 반사율 방정식에 기반하여 각각에 대한 근사치를 구하여 셰이더에 녹여내는 과정에 가깝습니다.

### BRDF (Bidirectional Reflective Distribution Function, 양방향반사도분포함수)

BRDF는 입사벡터(빛) $\omega_i$, 출사벡터(뷰) $\omega_o$, 표면 법선 $n$, 표면 파라미터 $a$ 를 통해 미세 표면의 거칠기를 표현하는 함수입니다. BRDF는 재질의 속성에 따라 들어오는 빛 $\omega_i$ 각각이 최종적으로 표면에서 반사되는 빛에 얼마나 기여하는지에 대해 근사합니다.

이를테면, 거울과 같이 완벽하게 매끄러운 표면의 경우, 출사각 $\omega_o$ 이 반사각과 완전히 동일하다면 1이 되고, 그외의 경우에는 0이 됩니다.
...
이제 이렇게 구한 Irradiance를 기반으로 Diffuse를 적용할 수 있게 되었습니다. 모델에 Diffuse와 베이스 컬러만을 통해 렌더링을 해보면, 환경 맵에 따라 적절한 Diffuse가 적용되는 것을 확인해볼 수 있습니다.
...
사실 반사광에 대해 정확하게 구현하고 사전 핉터링을 하기 위해서는 뷰 벡터 $V$ 에 대한 정보가 있어야 합니다. 하지만, 이 시점에서는 뷰 벡터에 대한 정보가 없기 때문에, 이를 단순화하기 위해 한 가지를 가정합니다. 바로 법선/반사/뷰 벡터가 모두 동일하다( $V=R=N$ )는 가정을 두어서 이를 근사하는 것입니다. 이를 통해 사전 필터링 단계에서 뷰 벡터에 대한 정보 자체가 필요없게 만듭니다.

이에 따라 완전한 구현과 비교하여 시각적인 차이는 생기지만, 충분히 용인 가능한 형태의 표면 반사를 그려낼 수 있습니다.

![V = R = N](image-16.png)

남은 항은 Specular 적분의 BRDF 부분인데, BRDF 역시 모든 방향에서 들어오는 Radiance가 완전한 백색이라는 가정 하에 미리 계산이 가능합니다.

BRDF는 roughness와 법선 벡터 $n$ 과 빛의 방향 벡터 $\omega_i$ 에 대한 각도를 input으로 구할 수 있습니다. 그 결과로 아래와 같이 R색상을 scale로, G색상을 bias로 사용하는 2D 텍스처가 도출됩니다. 

![BRDF LUT Example](image-17.png)

이제 이 사전 계산된 두 결과 텍스처들을 통해 아래와 같이 Specular를 근사할 수 있습니다.
